...
The supported Mega camera inventory types are 7 (video doorbell), 8 (HomeBase battery camera), 19 (T8160/S330), 31 (T8410/T8410C), 91 (T8213), 151 (Wired Wall Light Cam S100/T84A1), and 10031 (T817L). HomeBase children route PPCS through their parent. A parentless or self-parented supported camera routes through its own P2P DID, app connection, and DSK key. HomeBase type 18 is retained only as parent metadata and is not registered as a camera.
...
v0.1.22 adds Mega type 151 for the wired Wall Light Cam S100/T84A1 and the first direct PPCS route. It preserves the existing HomeBase path and accepts a standalone camera only when its own peer prerequisites are present. The lower-level direct media command is based on the existing first-party session implementation and still requires real S100 verification of discovery, live video, snapshots, and events.
