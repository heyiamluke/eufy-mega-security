...
The core lookup is [`camera-capability-core.ts`](../eufy_event_gateway/src/provider/camera-capability-core.ts). It has 11 rows: retained and fresh images, live video, clip retrieval, motion and person events, battery level, charging, health, and temperature. The first seven use existing gateway paths. The battery rows are discovery-only. A reported battery parameter is not a fresh value, and no battery measurement reaches Home Assistant yet. Known mains models that report a dummy battery level are suppressed.
