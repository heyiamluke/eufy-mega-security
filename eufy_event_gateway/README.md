...
It receives Eufy/HomeBase detections, retains the last useful image, and wakes supported battery cameras only for live view, a fresh snapshot, or a requested recording. It generates a private API token automatically and announces its connection details to Home Assistant through Supervisor discovery; its API port is not exposed to the LAN by default.
